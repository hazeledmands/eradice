...
export default function NaniteBackground() {
  ...
  useEffect(() => {
    ...
    function initCanvas() {
      ...
      // Fill void color once so trails fade to dark, not transparent
    ...
    function initParticles() {
      ...
      for (let ci = 0; ci < NUM_CLUSTERS; ci++) {
        const c = centers[ci];
        const count = ci < NUM_CLUSTERS - 1 ? perCluster : PARTICLE_COUNT - perCluster * (NUM_CLUSTERS - 1);
    ...
    function frame() {
      ...
      // Particles are sorted by cluster so we get ~30 batched fill() calls
      let lastColor = '';
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        const calmAngle = noise3d(p.x * NOISE_SCALE, p.y * NOISE_SCALE, t) * Math.PI * 2
          + (Math.random() - 0.5) * Math.PI * 0.5;
        // Chaotic angle: high-frequency noise + random jitter
        const chaoticAngle = noise3d(p.x * NOISE_SCALE * 18, p.y * NOISE_SCALE * 18, t * 6) * Math.PI * 2
        ...
        const speedMult = 1 + proximity * 4 + (((i * 2654435761) >>> 30) === 0 ? proximity * 6 : 0);
